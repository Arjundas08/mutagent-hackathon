...
/**
 * The ADL lifecycle stages in loop order (spec → build → evaluate → diagnose → optimize). Used ONLY
 * by the INTERNAL `status` profile (below) to record where a subject card currently sits in the loop.
 * It is NOT part of the public portable contract — a published card carries no lifecycle position.
 */
