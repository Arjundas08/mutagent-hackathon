...
//     locators only).
//
// NOTE: the `adl_stage` enum here (spec|build|evaluate|diagnose|optimize|audit) is the
// ROUTING-stage set. It intentionally differs from the TOPOLOGY `AdlStage` in
// sync-index.ts (which adds orchestrator|shared|unknown and omits optimize|audit):
...
// The CANONICAL inter-stage subject vocabulary is `agent | skill | code` — it
// matches config-schema `TargetSubject` (the widest shipped target set) so a
// `code` subject can flow through the routed HandoverBundle to *evaluate's
// failure routing. `code` (Wave-2 follow-up) is ADDITIVE: it only widens the
// closed union; every previously-valid `skill`/`agent` bundle still validates,
// and dispatch's `toSubjectKind` (topology → skill|agent) is unchanged — it
// simply never emits `code` (no topology entry is a code subject today), so the
// widening carries a code subject WITHOUT changing any existing producer.
// (optimize's `SubjectKind` additionally recognizes a `platform` goal-legality
// kind; `platform` is intentionally NOT added here — nothing produces a platform
// handover subject, and the closed union stays minimal.)
 ...
 *
 * NOTE (FU-69 §1.7): the LOCKED migration plan is "additive-then-one-breaking-cleanup, DEFERRED".
 * This is the additive half — `code` intentionally STAYS on `SubjectKind` for now. The deferred,
 * operator-gated cleanup (drop `code` from the SubjectKind union + bump HANDOVER_BUNDLE_VERSION) is
 * logged in the orchestrator decision ledger (ORCH-08); it is NOT done here.
 */
...
export const SubjectSchema = Type.Object(
  {
    ...
    // The SUBSTRATE axis (OPTIONAL, ADDITIVE — S15 / FU-69 §1.2). Added ALONGSIDE `kind`: a producer
    // may declare how the subject is realized so *evaluate can route by substrate (`artifactFormat ===
    // "code"`) rather than the conflated `kind === "code"`. Absent ⇒ infer from `kind` (back-compat:
    // every existing bundle still validates). The `code` value stays on `kind` too until the deferred
    // §1.7 cleanup (ORCH-08).
