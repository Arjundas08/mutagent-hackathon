 ...
 * the `agentspec.yaml` (def→impl cascade) — the ⑤ OPTIMIZE loop treats "converged"
 ...
 *
 * ADDITIVE — this is the SECOND axis, added ALONGSIDE `subject`. The `code` VALUE stays on `subject`
 * for now; the FU-69 §1.7 DEFERRED one-breaking cleanup (drop `code` from the TargetSubject union +
 * bump config_version) is operator-gated and logged in the orchestrator decision ledger (ORCH-08).
 */
...
export const TargetSchema = Type.Object(
  {
    ...
    // (full TDD in the worktree); the ⑤ OPTIMIZE loop's "converged" for it is BOTH
    ...
    // route by substrate (`artifact_format === "code"`) rather than the conflated `subject === "code"`.
    // Absent ⇒ infer from `subject` (back-compat: every existing config still validates). ApplyKind
    // derives from this (FU-69 §1.4). The `code` value stays on `subject` too until the deferred
    // §1.7 cleanup (ORCH-08).
